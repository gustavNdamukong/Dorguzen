	...
	public static function getView($viewName, \DGZ_library\DGZ_Controller $pageController = null, $format = null, $toolkit = false) {
		...
		else
		{
			...
			throw new \DGZ_library\DGZ_Exception(
				...
				. 'Please check that the class exists in either "' . $fileName . '" or in "' . $coreViewFile . '"'
			);
		}
		...
		if($object instanceof \DGZ_library\DGZ_HtmlView) {
			...
			if(!($pageController instanceof \DGZ_library\DGZ_Controller)) {
				throw new \DGZ_library\DGZ_Exception(
					'Controller Object Required for HtmlView',
					...
					. 'This is because HTML views may need to add their own styles and scripts into your controller object in order for them to work properly.'
				);
			}
	...
	public static function getAdminView($viewName, \DGZ_library\DGZ_Controller $viewController = null, $format = null) {
		...
		else
		{
			...
			throw new \DGZ_library\DGZ_Exception(
				...
				\DGZ_library\DGZ_Exception::NO_VIEW_FOUND,
				'No view class could be found called "' . $viewName . '" for format "' . '"' . PHP_EOL
				. 'Please check that the class exists in either "' . $fileName . '" or in "'
			);
		}
		...
		if($object instanceof \DGZ_library\DGZ_HtmlView) {
			...
			if(!($viewController instanceof \DGZ_library\DGZ_Controller)) {
				throw new \DGZ_library\DGZ_Exception(
					...
					'When creating a view which extends from //DGZ_library//DGZ_View, you must provide your DGZ_Controller object as the second parameter '
					. 'into DGZ_View::getView()' . PHP_EOL
					. '(got "' . (is_object($viewController) ? get_class($viewController) : (is_array($viewController) ? 'array' : print_r($viewController, true))) . '")'
					. 'This is because HTML views may need to add their own styles and scripts into your controller object in order for them to work properly.'
				);
			}
