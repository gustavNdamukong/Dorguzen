...
class blogPost extends \Dorguzen\Core\DGZ_HtmlView
{
    public function show(array $viewModel = []): void
    {
        ...
        <?php else: ?>
            <div style="background:linear-gradient(135deg,<?= $themeColor ?> 0%,#0d47a1 100%); padding:60px 0 40px;">
                <div class="container">
                    <?php if (!empty($post['category_name'])): ?>
                        <a href="<?= $rootPath ?>blog?category=<?= (int) $post['category_id'] ?>"
                           class="badge text-decoration-none mb-2 d-inline-block"
        ...
        <div class="container py-5">
            <div class="row">
                ...
                <div class="col-lg-8">
                    ...
                    <!-- Meta bar -->
                    <div class="d-flex align-items-center flex-wrap gap-3 mb-4 pb-3" style="border-bottom:1px solid var(--nlm-border, #e9ecef); font-size:.85rem; color:var(--nlm-text, #444);">
                        <span><i class="fa fa-user me-1"></i><?= htmlspecialchars($post['author']) ?></span>
                        <span><i class="fa fa-calendar me-1"></i><?= $postDate ?></span>
                        <a href="#comment-form" class="text-decoration-none" style="color:var(--nlm-link, <?= $themeColor ?>);">
                            <i class="fa fa-comments me-1"></i><?= count($comments) ?> comment<?= count($comments) !== 1 ? 's' : '' ?> — Leave one
                        </a>
                        <a href="<?= $rootPath ?>blog" class="ms-auto text-decoration-none" style="color:var(--nlm-link, <?= $themeColor ?>);">
                            <i class="fa fa-arrow-left me-1"></i>Back to blog
                        </a>
                    </div>

                    <!-- Body -->
                    <div class="blog-body" style="font-size:1.05rem; line-height:1.85; color:var(--nlm-text, #1a1a1a);">
                        <?= nl2br(htmlspecialchars($post['body'])) ?>
                    </div>

                    <!-- Comments -->
                    <div class="mt-5 pt-4" style="border-top:2px solid var(--nlm-border, #e9ecef);">
                        ...
                        <?php else: ?>
                            <?php foreach ($comments as $c): ?>
                                <div class="d-flex gap-3 mb-4 pb-3" style="border-bottom:1px solid var(--nlm-border, #f0f0f0);">
                                    ...
                                    <div>
                                        ...
                                        <p class="mb-0" style="font-size:.92rem; line-height:1.6; color:var(--nlm-text, #333);">
                        ...
                        <div id="comment-form" class="card border-0 shadow-sm p-4 mt-4" style="border-radius:12px;">
                            ...
                            <form method="post" action="<?= $rootPath ?>blog/comment">
                                ...
                                <!-- Math captcha -->
                                <div class="mb-4 p-3 rounded" style="background:#f8f9fa; border:1px dashed #dee2e6; color:#333;">
                                    <label class="fw-semibold mb-1" style="font-size:.85rem; color:#333;">
                                    ...
                                    <?php foreach ($recentPosts as $rp): ?>
                                        <li class="list-group-item border-0 py-3 <?= $rp['slug'] === $post['slug'] ? 'bg-light' : '' ?>">
                                            ...
                                            <?php if (!empty($rp['published_at'])): ?>
                                                <span style="font-size:.75rem; color:var(--nlm-text-muted, #555);">
