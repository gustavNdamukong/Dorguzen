...
class DGZ_Table
{
     ...
     *
     * This class has a dependency, and that is the Dorguzen\Core\DGZ_Dates class which is injected into the $_dateClass field
     * @param $data
     * @param int $count
    ...
    public function getTable($tableTemplateClassName, $sortLinkTarget = '') {
        ...
        if(file_exists($fileNameBase)) {
            ...
            $viewClass = 'Dorguzen\\Views\\'. $tableTemplateClassName;
        }
        elseif(file_exists($adminFileNameBase)) {
            include_once($adminFileNameBase);
            $viewClass = 'Dorguzen\\Views\\Admin\\'. $tableTemplateClassName;
        }
