...
abstract class DGZ_Layout
{
	...
	/**
	 * @var $showImageSlider whether or not to show an image slider for every view file.
	 */
	 ...
	 *
	 * @param bool $useFullLayout. Whether to use a partial or a full layout 
	 * @param $appName string. The name of this app so it knows how to scale the folder system for the right layout file
	 * @param $layoutFolder string. The name of the folder to find the layout in
	 * @param $layoutName string. The name of the layout to find
	 * @return object|null The name of layout if found
	...
	{
		$layoutConfig = container(Config::class);
		...
		if ($layoutConfig->getConfig()['live'] == 'false') {
			if ($useFullLayout) {
				...
				$layoutFileName = base_path($appRootPath);
