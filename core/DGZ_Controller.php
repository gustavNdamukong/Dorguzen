...
use Dorguzen\Modules\Seo\Controllers\SeoController;
use Dorguzen\Core\DGZ_Layout;
...
abstract class DGZ_Controller implements DGZ_Displayable {
    ...
    /**
     * @var object Dorguzen\Core\DGZ_Application An object which holds the detail of this application's configuration
     */
    ...
    /**
     * @var object \Dorguzen\Configs\Config Contains the configs of the app as views associated with controllers will need access to it
     */
    public $config;


    /**
     * @param DGZ_Translator object
     */
    public $translator;

    /**
     * @param DGZ_Validator object
     */
    public $validator;
    ...
    public function __construct()
    {
        ...
        $this->application = container(DGZ_Application::class);
        $this->config = $this->application->getAppConfig();
        $this->translator = container(DGZ_Translator::class);
        $this->validator = container(DGZ_Validator::class);
	...
	/**
	 * Gets the default layout for all pages. It gets this from the application (Config) class
	 * If you want your page to use a different one, override this function to return a different name.
	 ...
	 * Dorguzen\Core\DGZ_Layout's getLayout() which will in turn check this value when deciding the type of layout to use for the view.
    ...
    */ 
    ...
    protected function validator(array $data = [], array $rules = [], array $customMessages = []): DGZ_Validator
    {
        return $this->validator::make($data, $rules, $customMessages);
    }
    ...
    public function loadSeoData($viewName) { 
        if ($this->metadata == null)
        {
            ...
            if (
                ...
                ($modules['seo'] == 'on') &&
                class_exists('Dorguzen\Modules\Seo\Controllers\SeoController')
            )
            {
     ...
     *      of middleware checks until we crash.
	 * @throws Exception
	 */
	public function display($method, array $inputParameters, $needsMiddleware = true) 
    {
		try {
            ...
            if(count($this->warnings) > 0) {
                $warningView = DGZ_View::getView('WarningListView', $this);
                ob_start();
            ...
            if(count($this->errors) > 0) {
                $errorsView = DGZ_View::getView('ErrorsListView', $this);
                ob_start();
            ...
            if ($this->format == 'html') {
                try {
                    $layout = DGZ_Layout::getLayout(true, $this->appName, $this->defaultLayoutDirectory, $this->defaultLayout);
                    ...
                    // ExceptionView itself failed — last resort output
                    $isLocal = $this->config && ($this->config->getConfig()['live'] ?? 'true') === 'false';
                    if ($isLocal) {
                        echo '<pre style="color:red;background:#fff;padding:1em;">'
                            . '<b>' . get_class($e) . ':</b> ' . htmlspecialchars($e->getMessage())
     ...
     * the user refreshing/resubmitting the data. Here are some examples:
     * 
     *      $this->redirect('auth/login');
     *  or
     *      $this->redirect('auth', 'login');
     * 
     * This will redirect to the login() method of the AuthController. Make sure they exist.
     * 
     * If you are not within a controller context, you can call it on any instantiated controller like so:
     * 
     ...
     *
     * @param string $controller The name of the controller to target. Leave out the 'Controller' suffix 
     *  e.g. '' or 'home' for the HomeController 
     *      or 'auth' for the AuthController
     * @param string $method the method of the controller to call. Leave blank to run the default action of the controller
	...
	/**
     * NOTE: This assumes that all your controllers are in: 'src\controllers\'
     * Gets the name of a controller class.
     * A Controller name looks like: 'src\controllers\BamendaController', 
     *  so, first it strips of the 'Controller' at the end of the name,
     *  then it strips off the 'src\controllers\' prefix, & returns the string 'Bamenda'
     *  from the name
     * @return mixed
     */
    public function getNameFromController($controller = '')
	{
		...
		$shortName = strrchr($controller, '\\');
		$shortName = $shortName !== false ? ltrim($shortName, '\\') : $controller;
