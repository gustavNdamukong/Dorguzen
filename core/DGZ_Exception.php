...
class DGZ_Exception extends \Exception {
	...
	const FIELD_NOT_FOUND_ON_TABLE = 'fieldNotFoundOnTable';
	...
	public function __construct($message, $errorType = self::EXCEPTION, $hint = 'No hint available') {
		...
		if($errorType === self::NOT_IMPLEMENTED_EXCEPTION && $hint === 'No hint available') {
			$hint = 'The developer has either forgotten to, or has not implemented this functionality yet! :-)';
		}
	...
	public function display() {
		try {
			$controller = new \Dorguzen\Controllers\ExceptionController();
			$view = DGZ_View::getView('DGZExceptionView', $controller);
			$view->show($this);
		} catch (\Exception $e) {
			print $e->getMessage();
		}
