...
class DGZ_Validate
{
    ...
    public function validate_shop_name($shopName) {
        if ($shopName == "")
        {
            return '<p>No shop name entered</p>';
        }

        if (preg_match("/[^a-zA-Z0-9_-]/", $shopName))
        {
            return '<p>Only letters, numbers, -, and _ allowed for shop names. No spaces allowed</p>';
        }
        return "";
    }
}
