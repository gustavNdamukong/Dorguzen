...
class DGZ_Model
{
    ...
    public function __construct()
    {
        ...
        if ($this->config->getConfig()['live'] == 'false') {
     ...
     * TODO: THIS NEEDS IMPROVEMENT TO RUN QUERIES AS PREPARED STATEMENTS eg:
     * 
     *  $check = $goldMembers->query(
     *      "SELECT * FROM gold_members gm 
     *      WHERE gm.gold_members_users_id = ? 
     *      AND gm.gold_members_co_manager = ? 
     *      AND gm.gold_members_authorized = 0 
     *      AND gm.gold_members_authorized_by IS NULL",
     *      [$creatorId, $creatorId]);
     *  OR 
     * 
     ...
     * 
     * IDEALLY WE CAN MAKE IT ACCEPT A 2NG ARG, (ARRAY OF PLACEHOLDER VALUES), THEN CHECK FOR THIS & USE IT
     * IF PRESENT OR RUN AS IT CURRENTLY DOES IF THE ARRAY IS EMPTY.
     * 
     ...
     */
    public function selectWhere($columns = [], $criteria = [], $orderBy = '')
    {
        $model = new $this->whoCalledMe;
     ...
     */
    public function delete($table, $where = [], $dataTypes = '')
    {
        $db = $this->connect();
