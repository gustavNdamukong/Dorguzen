...
class DateConversion {
	...
	//convert a MySQL datetime string (hyphen-delimited, with time details at the end) into a PHP date string for display on a web page
	function displayDatetimeFromMySQL($date)
	{
		//global $months;

		$date_date = substr($date, 0, 10);
		$date_time = substr($date, -8);

		$old_data = explode('-', $date_date);
